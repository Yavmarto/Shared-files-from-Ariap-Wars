...
def check_tile_above(top_tile):
    if top_tile in cave_tiles.light_empty_bottom_canyon.values():
        return Bottom_Tile.empty_bottom
    # elif top_tile in cave_tiles.light_ground_bottom.values():
    #     return Bottom_Tile.ground_bottom
    elif top_tile in cave_tiles.light_ground_bottom_canyon.values():
        return Bottom_Tile.canyon_bottom
    # elif top_tile in cave_tiles.lightL_emptyR_bottom.values():
    #     return Bottom_Tile.groundL_emptyR_bottom
    elif top_tile in cave_tiles.lightL_emptyR_bottom_canyon.values():
        return Bottom_Tile.canyonL_emptyR_bottom
    # elif top_tile in cave_tiles.lightR_emptyL_bottom.values():
    #     return Bottom_Tile.groundR_emptyL_bottom
    elif top_tile in cave_tiles.lightR_emptyL_bottom_canyon.values():
        return Bottom_Tile.canyonR_emptyL_bottom

# Check the left tile
def check_tile_left(left_tile):
    if left_tile in cave_tiles.light_empty_right_canyon.values():
        return Left_Tile.empty_right
    # elif left_tile in cave_tiles.light_ground_right.values():
    #     return Left_Tile.ground_right
    elif left_tile in cave_tiles.light_ground_right_canyon.values():
        return Left_Tile.canyon_right
    # elif left_tile in cave_tiles.lightT_emptyB_right.values():
    #     return Left_Tile.groundT_emptyB_right
    elif left_tile in cave_tiles.lightT_emptyB_right_canyon.values():
        return Left_Tile.canyonT_emptyB_right
    # elif left_tile in cave_tiles.lightB_emptyT_right.values():
    #     return Left_Tile.groundB_emptyT_right
    elif left_tile in cave_tiles.lightB_emptyT_right_canyon.values():
        return Left_Tile.canyonB_emptyT_right

# get a tile based on the top 
def get_top(bottom, pos):
    if bottom == Bottom_Tile.empty_bottom:
            return general_map_methods.get_random_tile(cave_tiles.light_empty_top_canyon, pos)
    # elif bottom == Bottom_Tile.ground_bottom:
    #     return get_random_tile(cave_tiles.light_ground_top, pos)
    # elif bottom == Bottom_Tile.groundR_emptyL_bottom:
    #     return get_random_tile(cave_tiles.lightR_emptyL_top, pos)
    # elif bottom == Bottom_Tile.groundL_emptyR_bottom:
    #     return get_random_tile(cave_tiles.lightL_emptyR_top, pos)
    elif bottom == Bottom_Tile.canyon_bottom:
        return general_map_methods.get_random_tile(cave_tiles.light_ground_top_canyon, pos)
    elif bottom == Bottom_Tile.canyonR_emptyL_bottom:
        return general_map_methods.get_random_tile(cave_tiles.lightR_emptyL_top_canyon, pos)
    elif bottom == Bottom_Tile.canyonL_emptyR_bottom:
        return general_map_methods.get_random_tile(cave_tiles.lightL_emptyR_top_canyon, pos)
...
def get_left(left, tile):
    ...
    if left == Left_Tile.empty_right:
        return tile in cave_tiles.light_empty_left.values() or tile in cave_tiles.light_empty_left_canyon.values()
    # elif left == Left_Tile.ground_right:
    #     return tile in cave_tiles.light_ground_left.values()
    # elif left == Left_Tile.groundB_emptyT_right:
    #     return tile in cave_tiles.lightB_emptyT_left.values()
    # elif left == Left_Tile.groundT_emptyB_right:
    #     return tile in cave_tiles.lightT_emptyB_left.values()
    elif left == Left_Tile.canyon_right:
        return tile in cave_tiles.light_ground_left_canyon.values()
    elif left == Left_Tile.canyonB_emptyT_right:
        return tile in cave_tiles.lightB_emptyT_left_canyon.values()
    elif left == Left_Tile.canyonT_emptyB_right:
        return tile in cave_tiles.lightT_emptyB_left_canyon.values()
    return False
    ...
    for i in range(map_size_y):
        for j in range(map_size_x):
            ...
            if i == 0 or pos % GD_STEP_SIZE == 0:
                ...
                tile = general_map_methods.get_random_tile(cave_tiles.light_canyon_border, pos)
...
def add_ground_tiles(map):
    ...
    for i in range(len(map)):
        ...
        if ground_tile in cave_tiles.light_ground_border_tiles.values():
            ground_tile.insert(0, map[i][0])
            temp_ground_map.append(ground_tile)
            index_to_delete.append(i)
        elif ground_tile in cave_tiles.light_canyon_not_full.values():
            new_tile = [map[i][0]]
            new_tile.append(cave_tiles.light_cave_ground["full_1"][0])
            new_tile.append(cave_tiles.light_cave_ground["full_1"][1])
        ...
        for j in range(width):
            ...
            if i == 0 and j == 0:
                ...
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["lt"][0])
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["lt"][1])
            elif i == height-1 and j == width-1:
                # right bottom tile
                chosen_overlap_tile = []
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["rb"][0])
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["rb"][1])
            elif i == height-1 and j == 0:
                # left bottom tile
                chosen_overlap_tile = []
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["lb"][0])
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["lb"][1])
            elif i == 0 and j == width-1:
                # right top tile
                chosen_overlap_tile = []
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["rt"][0])
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["rt"][1])
            elif i == 0:
                chosen_overlap_tile = []
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["th"][0])
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["th"][1])
            elif j == 0:
                chosen_overlap_tile = []
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["lh"][0])
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["lh"][1])
            elif i == height-1:
                chosen_overlap_tile = []
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["bh"][0])
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["bh"][1])
            elif j == width-1:
                chosen_overlap_tile = []
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["rh"][0])
                chosen_overlap_tile.append(
                    cave_tiles.light_cave_ground["rh"][1])
